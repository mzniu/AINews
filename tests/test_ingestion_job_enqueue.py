...
from services.ingestion.job_enqueue import (
    enqueue_article_recrawl,
    enqueue_ingestion_job,
    find_active_article_recrawl_job,
...
def test_enqueue_article_recrawl_is_idempotent(tmp_path, monkeypatch):
    db_path = tmp_path / "enqueue_recrawl.db"
    monkeypatch.setenv("INGESTION_DATABASE_URL", f"sqlite:///{db_path.as_posix()}")
    init_db()
    session = get_session_factory()()
    session.add(
    ...
    job1, created1 = enqueue_article_recrawl(session, article_id="art1", source_id="src1")
    job2, created2 = enqueue_article_recrawl(session, article_id="art1", source_id="src1")

    assert created1 is True
    assert created2 is False
    assert job1 is not None and job2 is not None
    assert job1.id == job2.id
    assert job1.job_type == "article_recrawl"
    assert find_active_article_recrawl_job(session, "art1") is not None
