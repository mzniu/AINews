...
- 默认回退：`static/music/background.mp3`（本地生成/安装包，可能不在 Git 中）
...
`remotion/public/static` 符号链接到仓库 `static/`，因此 `Memories.mp3` 可直接作为 `audioPath: static/music/Memories.mp3`。

QA 与 `npm run render:bgm` 使用 `static/music/Memories.mp3`（通过 `test-bgm.mp3` 符号链接）。
