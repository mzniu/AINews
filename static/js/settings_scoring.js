(function () {
    ...
    function renderPresetOptions(data) {
        const select = $('scoringPresetSelect');
        if (!select) return;
        ...
        list.innerHTML = dimensions.map((dim) => {
            ...
            return `
                <div class="scoring-weight-row" data-key="${escapeAttr(dim.key)}">
                    ...
                    <input type="range" class="scoring-weight-slider" data-weight-key="${escapeAttr(dim.key)}"
                        min="0" max="40" step="1" value="${Math.round(value * 100)}">
                    <p class="hint">${escapeHtml(dim.hint || '')}</p>
                </div>
            `;
        }).join('');

        list.querySelectorAll('.scoring-weight-slider').forEach((slider) => {
            slider.addEventListener('input', () => {
                ...
                $('scoringPresetSelect').value = 'custom';
    ...
    function renderGrades(data) {
        ...
        $('gradeThresholdS').value = grades.S ?? 85;
        $('gradeThresholdA').value = grades.A ?? 70;
        $('gradeThresholdB').value = grades.B ?? 55;
        $('gradeThresholdC').value = grades.C ?? 40;
    }

    function collectWeights() {
        const weights = {};
        document.querySelectorAll('.scoring-weight-slider').forEach((slider) => {
    ...
    function collectGrades() {
        return {
            S: Number($('gradeThresholdS').value),
            A: Number($('gradeThresholdA').value),
            B: Number($('gradeThresholdB').value),
            C: Number($('gradeThresholdC').value),
        };
    }
    ...
    function applyPresetToForm(preset) {
        ...
        $('gradeThresholdS').value = grades.S ?? $('gradeThresholdS').value;
        $('gradeThresholdA').value = grades.A ?? $('gradeThresholdA').value;
        $('gradeThresholdB').value = grades.B ?? $('gradeThresholdB').value;
        $('gradeThresholdC').value = grades.C ?? $('gradeThresholdC').value;
        updateWeightSum();
    }
    ...
    async function loadScoringSettings() {
        setStatus('加载中…');
        const resp = await fetch('/api/ingestion/scoring/settings');
        const data = await resp.json();
        if (!resp.ok) throw new Error(data.detail || data.message || `HTTP ${resp.status}`);
        applyConfig(data);
        setStatus(data.has_local_override ? '已加载评分配置' : '使用默认评分模板（尚未保存本地覆盖）', 'ok');
    }

    async function saveScoringSettings(payload) {
        const resp = await fetch('/api/ingestion/scoring/settings', {
            method: 'PUT',
        ...
        applyConfig(data);
        dirty = false;
        setStatus(data.message || '评分配置已保存', 'ok');
        return data;
    }
    ...
    window.loadScoringSettings = loadScoringSettings;

    $('scoringPresetSelect')?.addEventListener('change', (event) => {
    ...
    ['gradeThresholdS', 'gradeThresholdA', 'gradeThresholdB', 'gradeThresholdC'].forEach((id) => {
        $(id)?.addEventListener('input', () => markDirty());
    });

    $('saveScoringBtn')?.addEventListener('click', () => {
        const preset = $('scoringPresetSelect').value;
        const payload = { grades: collectGrades() };
        if (preset === 'custom') {
        ...
        saveScoringSettings(payload).catch((e) => setStatus(e.message, 'error'));
    });

    $('reloadScoringBtn')?.addEventListener('click', () => {
        if (dirty && !window.confirm('有未保存的修改，确定重新加载？')) return;
        loadScoringSettings().catch((e) => setStatus(e.message, 'error'));
    });
})();
