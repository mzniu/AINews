"""Live smoke test for leiphone AI ingestion source."""
...
def main() -> int:
    ...
    with factory() as session:
        ...
        source = session.get(IngestionSource, "leiphone_ai")
        if source is None:
            print("FAIL: leiphone_ai source missing")
            return 1
        ...
        job = IngestionJob(job_type="manual", source_id="leiphone_ai", status="pending")
        session.add(job)
        session.flush()
        job_id = job.id
        session.commit()

    print("=== Live leiphone AI ingestion ===")
    try:
        with factory() as session:
            stats = IngestionOrchestrator(session).run_source("leiphone_ai", job_id=job_id)
    ...
    with factory() as session:
        ...
        rows = (
            ...
            .filter_by(source_id="leiphone_ai")
